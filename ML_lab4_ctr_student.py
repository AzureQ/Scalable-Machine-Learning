...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    index_list = sorted([OHEDict[f] for f in rawFeats])
    return SparseVector(numOHEFeats,index_list,[1 for i in range(0,len(index_list))])
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    items = point.split(",")
    label = items[0]
    raw_features = [(i, item) for i, item in enumerate(items[1:])]
    OHEfeatures = oneHotEncoding(raw_features,OHEDict,numOHEFeats)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    index_list = sorted([OHEDict[f] for f in rawFeats if f in OHEDict])
    return SparseVector(numOHEFeats,index_list,[1 for i in range(0,len(index_list))])
...
def evaluateResults(model, data):
    ...
    """
    return data.map(lambda a:computeLogLoss(getP(a.features,model.weights,model.intercept),a.label)).mean()
...
def parseHashPoint(point, numBuckets):
    ...
    raw_features = [(i, item) for i, item in enumerate(items[1:])]
    #OHEfeatures = oneHotEncoding(raw_features,OHEDict,numOHEFeats)
    hashFeatures = hashFunction(numBuckets,raw_features)
