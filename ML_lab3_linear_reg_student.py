...
def calcRMSE(labelsAndPreds):
    ...
    """
    return np.sqrt(labelsAndPreds.map(lambda (label,prediction):squaredError(label, prediction))
                           .mean())
...
def twoWayInteractions(lp):
    ...
    """
    interactions = list(itertools.product(range(len(lp.features)),range(len(lp.features))))
    return LabeledPoint(lp.label, np.hstack((lp.features,[lp.features[i] * lp.features[j] for (i,j) in interactions])))
